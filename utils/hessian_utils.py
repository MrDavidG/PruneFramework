...
def create_res_hessian_computing_tf_graph(input_shape, layer_kernel, layer_stride):
    ...
    patches = tf.extract_image_patches(images=input_holder,
                                       ...
                                       padding='SAME')
    print
    'Patches shape: %s' % patches.get_shape()
    ...
    print
    'Hessian shape: %s' % get_hessian_op.get_shape()
...
def generate_hessian(net, trainloader, layer_name, layer_type,
    ...
    for batch_idx, (inputs, _) in enumerate(trainloader):
        ...
        if batch_idx == 0:
            print
            '[%s] Now construct generate hessian op for layer %s' % (datetime.now(), layer_name)
            # res layer
            if layer_type == 'R':
                ...
                print
                'Kernel unfold shape: %d' % kernel_unfold_shape
            ...
            # linear layer
                ...
                weight_shape = net.state_dict()['module.%s.weight' % layer_name].size()
                print
                'Weights shape: %d' % weight_shape[1]
            ...
            print
            '[%s] %s Graph build complete.' % (datetime.now(), layer_name)
...
def generate_hessian_inv_Woodbury(net, trainloader, layer_name, layer_type, \
    ...
    for batch_idx, (inputs, _) in enumerate(trainloader):
        ...
        if batch_idx == 0:
            if layer_type == 'C' or layer_type == 'R':
                print
                '[%s] Now construct patches extraction op for layer %s' % (datetime.now(), layer_name)
                layer_input_np = layer_input.permute(0, 2, 3, 1).cpu().numpy()
            ...
            print
            '[%s] dataset: %d, input dimension: %d' % (datetime.now(), dataset_size, input_dimension)
